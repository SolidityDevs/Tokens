...
// File: contracts/BUSDSANTADividendPayingToken.sol
...
contract BUSDSANTA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address private _marketingWallet = address(0xd566a7a8e8B085D41cF5143e32513C8e070F91c5);
    // address private _devWallet = address(0xd566a7a8e8B085D41cF5143e32513C8e070F91c5);

    bool private swapping;
    bool public airdrop;

    BUSDSANTADividendTracker public dividendTracker;

    address public liquidityWallet;
    address public marketingWallet;
    // address public devWallet = _devWallet;

    uint256 public maxBuyTranscationAmount = 100000 * (10**18); // 1% of total supply
    uint256 public maxSellTransactionAmount = 100000 * (10**18); // 1% of total supply
    uint256 public swapTokensAtAmount = 50000 * (10**18);
    uint256 public _maxWalletToken = 200000 * (10**18); // 2%
    ...
    uint256 public immutable tradingEnabledTimestamp = 1636818057;
    ...
    constructor() public ERC20("BUSDSANTA", "BUSDSANTA") {
    	...
    	dividendTracker = new BUSDSANTADividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BUSDSANTA: The dividend tracker already has that address");

        BUSDSANTADividendTracker newDividendTracker = BUSDSANTADividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BUSDSANTA: The new dividend tracker must be owned by the BUSDSANTA token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUSDSANTA: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BUSDSANTA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BUSDSANTA: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BUSDSANTA: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUSDSANTA: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUSDSANTA: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BUSDSANTA: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BUSDSANTA: This account cannot send tokens until trading is enabled");
        }
...
contract BUSDSANTADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("BUSDSANTA_Dividend_Tracker", "BUSDSANTA_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "BUSDSANTA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BUSDSANTA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BUSDSANTA contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BUSDSANTA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BUSDSANTA_Dividend_Tracker: Cannot update claimWait to same value");
